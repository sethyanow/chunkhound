...
Phase 5 (ch-z2o) R8 requires graph-aware prompt templates in `chunkhound/services/research/prompts/graph_patterns/` that code_research's BFS uses for structural sub-questions. With ch-8mu complete, GraphWalkExpander provides graph-discovered chunks to `unified_search()`. This task adds templates that guide the QuestionGenerator to produce structural follow-up questions when the topic is structural.

The BFS exploration flow:
1. `BFSExplorationStrategy._explore_node()` calls `QuestionGenerator.generate_follow_up_questions()` (bfs_exploration_strategy.py:384)
2. QuestionGenerator uses `prompts.FOLLOWUP_GENERATION_USER` template (question_generator.py:210)
...
- `BFSExplorationStrategy._explore_node()` at `bfs_exploration_strategy.py:384` — calls question generator

Sub-epic says: "Pattern matching on question text, not hardcoded routing." This means the template system detects structural patterns in the GENERATED questions (post-LLM), not in the input query. Templates are prompt augmentations that steer the LLM toward structural questions.

## Design

Two changes:

1. **Create graph-aware prompt templates** — 5 template files in `chunkhound/services/research/prompts/graph_patterns/`:
...
Test: `test_graph_pattern_templates_exist` — import each template module, verify it exports `PATTERN` (str) and `PROMPT_AUGMENTATION` (str).

### Step 2: Create graph_patterns directory and 5 template files
Dir: `chunkhound/services/research/prompts/graph_patterns/`
...
- Import `ALL_PATTERNS` from `chunkhound.services.research.prompts.graph_patterns`
...
## Success Criteria
- [ ] 5 template files exist in `chunkhound/services/research/prompts/graph_patterns/`
