...
- [2026-04-26T19:42:34Z] [Seth Yanow] FIXED. Refactor landed at commit f120462 on dev. Extracted _attempt_batch nested helper; lock now scoped to a single attempt; recursion happens outside semaphore. New _BATCH_SPLIT_NEEDED sentinel signals split-needed without conflating with int return values. 5 tests in test_ch_qw4_semaphore_deadlock.py (2 regression + 3 adversarial: singular, multi-level recursion, second-run). Re-locked TestRecursiveTokenLimitSplitPreservesTaskPassage to max=1; max=2 workaround removed. Pre-commit gate 3063 passed, 44 skipped, 7min runtime. mypy clean on file. A3 invariant verified: rg shows recursion only at lines 601-602, OUTSIDE async-with-semaphore at line 589-592.
- [2026-04-26T19:44:43Z] [Seth Yanow] Debrief: clean refactor, no workarounds. Surprise — Test 2 RED needed await asyncio.sleep(0) in mock side_effect to force concurrent permit-holding (asyncio scheduling subtlety not anticipated by plan or SRE). Fixed in same cycle, deadlock then surfaced cleanly. Skeleton + SRE clarifications were highly accurate; only one mid-cycle test correction needed (the sleep). 5 tests landed (2 regression + 3 adversarial). Reflections: write to MEMORY.md done — updated semaphore deadlock entry to FIXED + added new tooling quirk entry on AsyncMock side_effect needing internal await. No user mid-task corrections.
